...
export async function getNearbyHealthFacilities(lat, lng, radius = 5, type = "doctors") {
  const finalLat = lat || 30.3762;
  const finalLng = lng || 76.1427;
  ...
  try {
    ...
    const url = `https://api.geoapify.com/v2/places?categories=${category}&filter=circle:${finalLng},${finalLat},${radius * 1000}&bias=proximity:${finalLng},${finalLat}&limit=10&apiKey=${apiKey}`;
    ...
    const mockFacilities = [
      { id: "m1", name: "City Medical Center", specialty: "General", lat: finalLat + 0.01, lng: finalLng + 0.01, distance: "1.2", type: "clinic" },
      { id: "m2", name: "Metro General Hospital", specialty: "Emergency", lat: finalLat - 0.01, lng: finalLng - 0.01, distance: "2.5", type: "hospital" }
    ];
