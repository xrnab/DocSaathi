  ...
  try {
    ...
    const appointments = await db.appointment.findMany({
      where: {
        doctorId: doctor.id,
        status: {
          in: ["SCHEDULED", "COMPLETED", "CANCELLED", "REJECTED"],
        },
      },
      include: {
        patient: true,
...
/**
 * Reject an appointment (only by doctor)
 * Refunds credits to patient and deducts from doctor
 */
export async function rejectAppointment(formData) {
  const { userId } = await auth();
  ...
  try {
    ...
    if (appointment.status !== "SCHEDULED") {
      throw new Error("Only scheduled appointments can be rejected");
    }
    ...
    await db.$transaction(async (tx) => {
      ...
      // Refund credits to patient (increment)
      await tx.user.update({
        where: {
          id: appointment.patientId,
        },
      ...
      // Deduct credits from doctor (decrement)
      await tx.user.update({
        where: {
          id: appointment.doctorId,
        },
      ...
      await tx.creditTransaction.create({
        data: {
          userId: appointment.patientId,
          amount: 2,
          type: "ADMIN_ADJUSTMENT", // Using adjustment for refund
        },
      });
...
export async function markAppointmentCompleted(formData) {
  ...
  try {
    ...
    const appointmentId = formData.get("appointmentId");
    const grading = formData.get("grading");
    const improvementStatus = formData.get("improvementStatus");
    const doctorSummary = formData.get("doctorSummary");
    ...
    // Update the appointment status to COMPLETED and add feedback
    const updatedAppointment = await db.appointment.update({
      where: {
        id: appointmentId,
      },
